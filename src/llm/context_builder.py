...
def build_context(results_dir: str | Path) -> dict:
    """
    Build a structured context dict from saved outputs.

    This context is the single source of truth for the LLM layer.
    The LLM should explain these results, not invent new ones.
    """
    ...
    # Forecasting
    # ------------------------------------------------------------------
    w1 = load_json_safe(results_dir / "week1_baseline_results.json")
    if isinstance(w1, list):
        baseline_test = next((r for r in w1 if r.get("evaluation_split") == "test"), None)
        if baseline_test:
            context["baseline_forecasting"] = {
                "model": "Seasonal Naive (weekly seasonality)",
                "test_rmse": _round_or_none(baseline_test.get("rmse")),
                "test_mae": _round_or_none(baseline_test.get("mae")),
                "test_mape": _round_or_none(baseline_test.get("mape"), 2),
            }

    w2 = load_json_safe(results_dir / "week2_forecasting_results.json")
    if isinstance(w2, list) and len(w2) > 0:
        ...
        context["advanced_forecasting"] = models

        baseline_rmse = context.get("baseline_forecasting", {}).get("test_rmse")
        lgbm_point = next((r for r in models if r.get("model") == "LightGBM Point"), None)
        if lgbm_point:
            improvement = None
            if baseline_rmse and lgbm_point.get("rmse"):
                improvement = round((1 - lgbm_point["rmse"] / baseline_rmse) * 100, 1)
        ...
        if lgbm_quant:
            ...
            context["probabilistic_forecasting"] = {
                ...
                "interpretation": (
                    f"{cov * 100:.1f}% of actual values fell inside the 90% prediction interval."
    ...
    if isinstance(did, dict) and did:
        ...
        context["causal_did"] = {
            ...
            "n_obs": did.get("n_obs"),
            "interpretation": (
                f"Promotions causally increased demand by {estimate:+.3f} units/day "
                f"(95% CI [{ci_low:+.3f}, {ci_high:+.3f}], p={p_value:.4f})."
                if None not in (estimate, ci_low, ci_high, p_value)
                else "Difference-in-Differences results are available but incomplete."
            ),
        }
    ...
    if isinstance(placebo, dict) and placebo:
        ...
        context["causal_placebo_test"] = {
            ...
            "passed": passed,
            "interpretation": (
                "Placebo test passed, suggesting the DiD estimate is less likely to be driven by pre-existing trends."
                if passed is True else
                "Placebo test did not pass cleanly, so the DiD estimate should be interpreted cautiously."
                if passed is False else
                "Placebo test result is available but incomplete."
            ),
        }
    ...
    if isinstance(naive, dict) and naive and "causal_did" in context:
        ...
        context["selection_bias"] = {
            ...
            "bias_pct": bias_pct,
            "interpretation": (
                f"The naive promotion estimate overstated the effect by about {bias_pct}% relative to the causal estimate."
                if bias_pct is not None else
                "Selection-bias comparison is available but incomplete."
            ),
        }
    ...
    if store_hte is not None and not store_hte.empty:
        context["store_promotion_sensitivity"] = {
            ...
            "interpretation": "Stores ranked by estimated heterogeneous treatment effect of promotions.",
        }
    ...
    if item_hte is not None and not item_hte.empty:
        context["item_promotion_sensitivity"] = {
            ...
            "interpretation": "Items ranked by estimated heterogeneous treatment effect of promotions.",
        }

    # ------------------------------------------------------------------
    # Elasticity / promotion sensitivity
    # ------------------------------------------------------------------
    ...
    if isinstance(panel, dict) and panel:
        ...
        context["panel_elasticity"] = {
            ...
            "interpretation": (
                f"After controlling for item fixed effects, promotions increased demand by about "
                f"{_round_or_none(panel.get('pct_demand_change'), 2):+.2f}% on average."
                if panel.get("pct_demand_change") is not None else
                "Panel promotion-sensitivity results are available but incomplete."
            ),
        }

    family_df = load_csv_safe(results_dir / "promotion_sensitivity_by_family.csv")
    if family_df is not None and not family_df.empty:
        if "significant" in family_df.columns:
            sig_df = family_df[family_df["significant"] == True].copy()
        else:
            sig_df = family_df.copy()

        if "pct_demand_change" in sig_df.columns:
            sig_df = sig_df.sort_values("pct_demand_change", ascending=False).reset_index(drop=True)

        context["family_elasticity"] = {
    ...
    if isinstance(anomaly_results, list) and len(anomaly_results) > 0:
...
def format_context_for_prompt(context: dict) -> str:
    """
    Convert structured context into a compact prompt string.
    """
    lines: list[str] = []
    ...
    if "selection_bias" in context:
        ...
        lines.append("SELECTION BIAS ANALYSIS:\n")
        lines.append(
    ...
    if "panel_elasticity" in context:
        pe = context["panel_elasticity"]
        lines.append("PROMOTION SENSITIVITY (PANEL MODEL):\n")
        lines.append(
            f"- Method: {pe['method']}\n"
    ...
    if "family_elasticity" in context:
        fe = context["family_elasticity"]
        ...
        if fe.get("top_5_families"):
            ...
            for row in fe["top_5_families"]:
                lines.append(
                    f"  - {row.get('family', 'N/A')}: "
                    f"{row.get('pct_demand_change', 'N/A')}% demand change\n"
                )
        lines.append("\n")
    ...
    if "store_promotion_sensitivity" in context:
        ...
        for row in sh.get("top_stores", []):
            lines.append(
                f"  - Store {row.get('store_nbr', '?')}: "
                f"{row.get('promotion_lift_estimate', 'N/A')} units/day\n"
            )
        lines.append("\n")
    ...
    if "anomaly_detector" in context:
        ...
        lines.append("VISUAL ANOMALY DETECTOR:\n")
        lines.append(
